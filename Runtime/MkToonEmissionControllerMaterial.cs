...
{
    public class MkToonEmissionControllerMaterial : ColorController
    {
        ...
        private void Awake()
        {
            ...
            {
                _originalColors[i] = Properties.emissionColor.GetValue(_materials[i].GetMaterial());
            }
        }
        ...
        private void ApplyOverlay(Color? overlay, float weight)
        {
            for (int i = 0; i < _materials.Length; i++)
            {
                if (overlay.HasValue)
                {
                    Properties.emissionColor.SetValue(GetMaterial(i),
                        ColorUtils.LerpColorsByWeights(_originalColors[i], OriginalColorWeight, overlay.Value, weight));
                }
                else
                {
                    Properties.emissionColor.SetValue(GetMaterial(i), _originalColors[i]);
                }
            }
        }
